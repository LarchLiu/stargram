...
export default eventHandler(async (event) => {
  ...
  if (method === 'GET') {
    ...
    const appConfig = await getBotConfig('stargram') as BotConfig
    if (!appId)
    ...
    if (_thisConfig) {
      if (!userList.includes(userId)) {
        ...
        await setBotConfig('stargram', appConfig)
      }
      const thisConfig = JSON.parse(cryption.decode(_thisConfig)) as ServerConfig<OutUserConfig>
      const _userConfig: any = await getUserConfig('stargram', appId, userId) || {}
      ...
      await setUserConfig('stargram', appId, userId, _userConfig)
      return 'ok'
    }
    else {
      setResponseStatus(event, 400)
      return { error: 'App No Config' }
    }
  }
})
