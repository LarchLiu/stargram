...
export default {
  ...
  command: {
    ...
    role: {
      ...
      current_defined_role: (size: string) => `当前已定义的角色如下(${size}):\n`,
      ...
      delete_role_error: (e: any) => `删除角色错误: \`${e.message}\``,
      update_role_success: '更新配置成功',
      update_role_error: (e: any) => `配置项格式错误: \`${e.message}\``,
    },
    img: {
    ...
    new: {
      ...
      new_chat_start_private: (id: string) => `新的对话已经开始，你的ID(${id})`,
      new_chat_start_group: (id: string) => `新的对话已经开始，群组ID(${id})`,
    },
    setenv: {
      help: '配置项格式错误: 命令完整格式为 /setenv KEY=VALUE',
      update_config_success: '更新配置成功',
      update_config_error: (e: any) => `配置项格式错误: ${e.message}`,
    },
    adduser: {
      help: '添加用户有误: 完整命令格式为 /adduser UserId',
      update_config_success: '添加用户成功',
      add_user_error: (e: any) => `添加用户有误: ${e.message}`,
    },
    version: {
      new_version_found: (current: any, online: any) => `发现新版本，当前版本: ${JSON.stringify(current)}，最新版本: ${JSON.stringify(online)}`,
      current_is_latest_version: (current: any) => `当前已经是最新版本, 当前版本: ${JSON.stringify(current)}`,
    },
    usage: {
      usage_not_open: '当前机器人未开启用量统计',
      current_usage: '📊 当前机器人用量\n\nTokens:\n',
      total_usage: (total: number) => `- 总用量：${total || 0} tokens\n- 各聊天用量：`,
      no_usage: '- 暂无用量',
    },
    permission: {
      not_authorized: '身份权限验证失败',
      not_enough_permission: (roleList: any, chatRole: any) => `权限不足,需要${roleList.join(',')},当前:${chatRole}`,
      role_error: (e: any) => `身份验证出错:${e.message}`,
      command_error: (e: any) => `命令执行错误: ${e.message}`,
    },
  },
}
